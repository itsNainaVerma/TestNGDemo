...
Hard Assert Fail scenarios

-->A hard assert stops execution after a fail and 
moves on to the next annotation.<--
*/
...
public class VerifyHardAssertFail
{
		...
		public void testHomePageVerification() {
			Assert.assertEquals(true,false,"Welcome link is not correct on home page");
			System.out.println("3. Verify Welcome Link");
			
			//keep below assert o/p as PASS to check behaviour of hard assert
			//execution will stop here because we have used hard assert
			Assert.assertFalse(false, "The admin tab is not displayed");
			System.out.println("4. Verify Admin Tab");
			 
			Assert.assertEquals(true, false);
			
			Assert.assertTrue(false,"The dashboard is not corrrect");
